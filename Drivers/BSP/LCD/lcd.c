...
static uint8_t swap_x_y;
static uint8_t rev_y;
static uint8_t rev_x;
static void lcd_xy_convert(uint16_t *x, uint16_t *y)
{
	uint16_t temp;
...
uint32_t lcd_read_point(uint16_t x, uint16_t y)
{
	...
	if(lcddev.id == 0X5408)
	{	//BGR --> RGB
		return (r&0x001F)<<11 | (r&0x07E0) | (r&0xF800)>>11;
	}
...
void lcd_scan_dir(uint8_t dir)
{
	...
	if(lcddev.id == 0x5408)
	{
		...
		_regval |= (uint16_t)1<<12;
		lcd_write_reg(0x03, _regval);
	...
	else if (lcddev.id != 0X1963)   /* 1963不做坐标处理 */
	{
		if (regval & 0X20)
		{
...
void lcd_set_window(uint16_t sx, uint16_t sy, uint16_t width, uint16_t height)
{
   ...
   if (lcddev.id == 0X1963 && lcddev.dir != 1)     /* 1963竖屏特殊处理 */
	...
	{
		lcd_xy_convert(&sx, &sy);
		lcd_xy_convert(&twidth, &theight);
		if(sx > twidth) {
			x_min = twidth;
			x_max = sx;
		} else {
		...
		//SetWindow
		lcd_write_reg(0x50, x_min);	//Set X Star
		lcd_write_reg(0x51, x_max);	//Set X End
		lcd_write_reg(0x52, y_min);	//Set Y Star
		lcd_write_reg(0x53, y_max);	//Set Y End

		//SetCursor
		lcd_write_reg(lcddev.setxcmd, sx);
		lcd_write_reg(lcddev.setycmd, sy);		
	}
...
void lcd_init(void)
{
    ...
    fsmc_write_handle.DataSetupTime = 2;        /* 数据保存时间(DATAST)为2个HCLK = 13.9 * 2 = 27.8ns */
